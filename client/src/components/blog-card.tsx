...
export default function BlogCard({ post }: BlogCardProps) {
  ...
  return (
    <article className="group border-b border-white/10 pb-8 last:border-b-0" data-testid={`blog-card-${post.id}`}>
      <div className="flex items-start gap-6">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm text-white/70 font-medium">{author.name}</span>
            {author.verified && (
            ...
            <span className="text-sm text-white/50">
              in <span className="font-medium text-white/70">{post.category || 'Design'}</span>
            </span>
          </div>

          {/* Title */}
          <h2 className="text-2xl font-bold text-white leading-tight group-hover:text-white/90 transition-colors">
            <Link 
              href={`/blog/${post.id}`}
          ...
          {/* Excerpt */}
          <p className="text-white/70 text-lg leading-relaxed line-clamp-2" data-testid={`blog-excerpt-${post.id}`}>
            {post.excerpt}
          </p>
